...
# Функция для получения байтов картинки
        ...
        for _ in range(15):
            ...
            if data and data.get("resultJson"):
                ...
                if url:
                    ...
                    return BytesIO(img_resp.content) # Возвращаем картинку как поток данных
...
async def generate(message: Message):
    topic = message.text
    msg = await message.answer("⏳ Генерирую картинку и собираю файл...")
    
    async with httpx.AsyncClient(timeout=300.0) as client:
        # 1. Генерируем картинку
        img_io = await get_image_bytes(client, f"Professional cover for a presentation about {topic}")
        
        # 2. Генерируем структуру презентации
        prompt = f"Создай структуру из 3 слайдов на тему '{topic}'. Ответ строго JSON: [{{'title': 'Заголовок', 'content': 'Текст'}}, ...]"
        resp = await client.post(CHAT_API_URL, json={"messages": [{"role": "user", "content": prompt}]}, 
                               headers={"Authorization": f"Bearer {API_KEY}", "Content-Type": "application/json"})
        
        raw_text = resp.json()['choices'][0]['message']['content'].replace("```json", "").replace("```", "").strip()
